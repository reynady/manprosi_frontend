...
async function getValveById(id: string) {
  ...
  if (!json?.success) throw new Error(json?.error || "Failed to fetch valve");
  return json.data;
}

async function deleteValve(id: number) {
  const json = await fetchJson(`${API_URL}/valves/${id}`, { method: "DELETE" });
  if (!json?.success) throw new Error(json?.error || "Failed to delete valve");
  return json;
}
