...
async function getAutomationById(id: string) {
    const json = await fetchJson(`${API_URL}/automations/${id}`);
    return json?.data;
}

// 2. Delete Automation
async function deleteAutomation(id: number) {
    const json = await fetchJson(`${API_URL}/automations/${id}`, { method: "DELETE" });
    return json;
}

// 3. Get Automation History
async function getAutomationHistory(automationId: string) {
    const json = await fetchJson(`${API_URL}/automations/${automationId}/history`);
    return json?.data;
}
